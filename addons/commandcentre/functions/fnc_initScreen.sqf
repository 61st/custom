/*
Function: sixtyone_commandcenter_fnc_initScreen
...
if (isNil "SIXTYONE_CC_nextScreenID") then {
	SIXTYONE_CC_nextScreenID = 0;
};

_object setVariable [format ["SIXTYONE_CC_screen_%1_ID", _selection], SIXTYONE_CC_nextScreenID, true];

SIXTYONE_CC_nextScreenID = SIXTYONE_CC_nextScreenID + 1;

_object setVariable [format ["SIXTYONE_CC_screen_%1_on", _selection], false, true];
_object setVariable [format ["SIXTYONE_CC_screen_%1_mode", _selection], "", true];
_object setVariable [format ["SIXTYONE_CC_screen_%1_target", _selection], "", true];

// set default screen texture
_object setObjectTextureGlobal [_selection, "z\sixtyone\Addons\Media\images\cc_screen_standby.paa"];

[_object, _selection, _name, [_allowCam, _allowDrone, _allowSat, _allowMap]] remoteExec ["sixtyone_commandcenter_fnc_addScreenActions", 0, true];