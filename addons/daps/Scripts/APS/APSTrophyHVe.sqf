...
while{TRUE}do{
    ...
    if(_v call DAPS_fnc_Active)then{
        ...
        _r=_r-_reg;
        if((count _r)>0)then{_1=_r select 0;if(_1 in _s)then{_sh=TRUE};[_v,_1,_sh]spawn DAPS_fnc_TrophyHVe;_l=.5;_sh=FALSE;_reg pushBack(_r select 0)};
        if((count _r)>1)then{_2=_r select 1;if(_2 in _s)then{_sh=TRUE};[_v,_2,_sh]spawn DAPS_fnc_TrophyHVe;_l=.5;_sh=FALSE;_reg pushBack(_r select 1)};
