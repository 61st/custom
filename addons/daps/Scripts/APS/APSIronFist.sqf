...
while{TRUE}do{
    ...
    if(_v call DAPS_fnc_Active)then{
        ...
        _r=_r-_reg;
        if((count _r)>0)then{[_v,_r select 0]spawn DAPS_fnc_IronFist;_l=.5;_reg pushBack(_r select 0)};
        if((count _r)>1)then{[_v,_r select 1]spawn DAPS_fnc_IronFist;_l=.5;_reg pushBack(_r select 1)};
