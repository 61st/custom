...
private _mapCtrl = ((findDisplay IDC_SIXTYONE_ADMINMENU_RESP_MAP_DISPLAY) displayCtrl IDC_SIXTYONE_ADMINMENU_RESP_MAP_CONTROL);//_this select 0;//
private _pos = [0,0,0];

if (alive player && !(player isKindOf "sixtyone_spectator_unit")) then {
    _pos = getPos player;  
} else {
