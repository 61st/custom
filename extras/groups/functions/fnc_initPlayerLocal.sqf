...
if (isServer && !hasInterface) then {
	LXIM_isACEEnabled = isClass (configFile >> "CfgPatches" >> "ace_common");
	LXIM_arsenalBoxes = [arsenal_1];

	player addEventHandler ["Respawn",FUNC(onRespawn)];

	if (LXIM_isACEEnabled) then {
		private _groupCategory = [
			...
			{[] call FUNC(initGroupMenu)},
			{
				...
				{(LXIM_arsenalBoxes findIf {(player distance _x) < 50}) isNotEqualTo -1}
			}
		...
		["ace_arsenal_displayClosed",{
			LXIM_savedLoadout = getUnitLoadout player;
		}] call CBA_fnc_addEventHandler;
	} else {
		{
			_x addAction ["Open Group Menu",FUNC(initGroupMenu)];
		} forEach LXIM_arsenalBoxes;
